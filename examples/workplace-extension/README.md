...
The backend depends on `skein-agents>=0.3.0,<0.7.0`. The distribution installs the public `app.*` imports.
...
4. It installs `skein-agents` 0.6.4 and Atlas 2.0 from new wheels.
...
uv pip install --python .venv-test/bin/python --no-deps \
  dist/skein_agents-0.6.4-py3-none-any.whl \
