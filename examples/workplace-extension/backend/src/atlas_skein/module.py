...
from app.public import CommandContext, PublicError, WorkItems
...
def atlas_module(
    ...
    def run_sync(work_items: WorkItems, command_context: CommandContext) -> dict[str, int]:
        try:
            return integration.sync(work_items, command_context)
            ...
            raise PublicError(
                "ATLAS_BAD_RESPONSE",
                "The Atlas synchronization service returned an unusable response.",
                status_code=502,
            ) from exc
    ...
    def notify(context: WorkflowActionContext, request: BaseModel) -> NotifyOut:
        ...
        try:
            ...
            raise PublicError(
                "NOTIFICATION_BAD_RESPONSE",
                "The Atlas notification service returned an unusable response.",
                status_code=502,
            ) from exc
    ...
    return SkeinModule(
        ...
        tools=(
            ToolContribution(
                ...
                timeout_seconds=20,
                error_codes=("ATLAS_UNAVAILABLE", "ATLAS_BAD_RESPONSE"),
            ...
            MigrationContribution(
                ...
                (
                    ...
                    ExtensionMigration(
                        ...
                        "lease-status-outbox",
                        (
                            # BY DEFAULT permits the ordered v4 backfill. This migration
                            # restores ALWAYS before application writes resume.
                            "ALTER TABLE status_outbox ADD COLUMN sequence_id bigint"
                            " GENERATED BY DEFAULT AS IDENTITY",
                            "WITH ordered AS (SELECT event_id, row_number() OVER"
                            " (ORDER BY event_id) AS position FROM status_outbox)"
                            " UPDATE status_outbox AS target SET sequence_id = ordered.position"
                            " FROM ordered WHERE target.event_id = ordered.event_id",
                            "ALTER TABLE status_outbox ALTER COLUMN sequence_id"
                            " SET GENERATED ALWAYS",
                            "ALTER TABLE status_outbox ADD COLUMN lease_token TEXT"
                            " NOT NULL DEFAULT ''",
                            "ALTER TABLE status_outbox ADD COLUMN lease_until"
                            " timestamp with time zone",
                            "ALTER TABLE status_outbox ADD COLUMN dead INTEGER NOT NULL DEFAULT 0",
                            "ALTER TABLE status_outbox ADD COLUMN error_code TEXT"
                            " NOT NULL DEFAULT ''",
                            "CREATE UNIQUE INDEX status_outbox_sequence_id"
                            " ON status_outbox (sequence_id)",
                            "CREATE INDEX status_outbox_pending_order"
                            " ON status_outbox (sequence_id)"
                            " WHERE delivered = 0 AND dead = 0",
                            "CREATE INDEX status_outbox_pending_item_order"
                            " ON status_outbox (external_id, sequence_id)"
                            " WHERE delivered = 0 AND dead = 0",
                        ),
                    ),
                ),
        ...
        workflow_actions=(
            WorkflowActionContribution(
                ...
                timeout_seconds=10,
                error_codes=("NOTIFICATION_UNAVAILABLE", "NOTIFICATION_BAD_RESPONSE"),
            ),
        ),
    )
