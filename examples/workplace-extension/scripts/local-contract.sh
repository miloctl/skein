...
expected = {
    "skein_agents-0.6.0-py3-none-any.whl",
    "miloctl-skein-extension-api-1.0.0.tgz",
    "miloctl-skein-frontend-host-0.6.0.tgz",
}
entries = {}
...
PY
    cp "$artifact_root/skein_agents-0.6.0-py3-none-any.whl" "$artifacts/"
    cp "$artifact_root/miloctl-skein-extension-api-1.0.0.tgz" "$artifacts/"
    cp "$artifact_root/miloctl-skein-frontend-host-0.6.0.tgz" "$artifacts/"
fi
...
for artifact in \
    ...
    miloctl-skein-extension-api-1.0.0.tgz \
    miloctl-skein-frontend-host-0.6.0.tgz; do
...
UV_CACHE_DIR="$tmp/uv-cache" uv pip install --quiet \
    ...
    "$artifacts/skein_agents-0.6.0-py3-none-any.whl" \
