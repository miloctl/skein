...
KEPT = {
    ...
    "github_recovery_api": "one shared throttle deadline per approved API destination; restart must not reset it",
    "github_recovery_hooks": "one checkpoint and unresolved history-gap record per configured hook",
    "github_recovery_deliveries": "permanent GUID success evidence; an older failed attempt must not trigger replay",
    "github_recovery_attempts": "redelivery request outcomes, including unknown external effects; operator recovery evidence",
