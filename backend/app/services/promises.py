...
def add_promise(
    ...
    *,
    ...
    db.log_activity(actor, "add_promise", f"#{cid} {promise[:80]}")
...
def update_promise(
    ...
    if not row:
        raise db.NotFound(f"promise #{promise_id} not found")
    if row["status"] != "open":
...
def edit_promise(
    ...
    *,
    ...
    row = db.query_one("SELECT promise, status FROM promises WHERE id = ?", (promise_id,))
    if not row:
        raise db.NotFound(f"promise #{promise_id} not found")
    if row["status"] != "open":
    ...
    if promise and promise != row["promise"]:
        db.log_activity(actor, "edit_promise", f"#{promise_id}: '{row['promise']}' -> '{promise}'")
    else:
        db.log_activity(actor, "edit_promise", f"#{promise_id} {' '.join(fields)}")
    return {"id": promise_id, "updated": list(fields)}


def list_promises(status: str = "", audience: str = "") -> list[dict]:
    where, params = [], []
    if status:
        where.append("status = ?")
    ...
    return db.query(
        f"SELECT * FROM promises{clause}"  # noqa: S608 — clauses hardcoded
