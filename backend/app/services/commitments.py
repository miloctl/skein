...
def add_commitment(promise: str, to_whom: str = "", due_date: str = "",
    ...
    cid = db.execute(
        ...
        " VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (promise, to_whom, engagement_id or None, due_date or None,
         origin, actor, ts, ts),
    )
...
def update_commitment(commitment_id: int, status: str,
                      *, actor: str = "system", origin: str = "human") -> dict:
    ...
    db.execute("UPDATE commitments SET status = ?, updated_at = ? WHERE id = ?",
...
def list_commitments(status: str = "") -> list[dict]:
    if status:
        return db.query(
            "SELECT * FROM commitments WHERE status = ?"
            " ORDER BY due_date IS NULL, due_date, id", (status,))
    return db.query(
