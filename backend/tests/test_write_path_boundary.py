...
@pytest.mark.parametrize("layer", ["routes", "tools"])
def test_no_mutating_db_helpers_outside_the_service_layer(layer):
    helpers = re.compile(r"\bdb\.(?:execute|execute_rowcount|claim_job|log_activity)\(")
    offenders = [
        f"{path.relative_to(APP.parent)}:{line_no}: {line.strip()}"
