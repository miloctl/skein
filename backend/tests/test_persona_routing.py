...
def test_negative_prompts_route_to_their_owner():
    """A negative is an ask that sounds adjacent — the named owner must
    outrank the case's persona, or the two descriptions blur."""
    docs = _bench_docs()
    misses = []
    for slug, case in _load_cases().items():
        for neg in case.get("negative", []):
            ranking = _rank(neg["prompt"], docs)
            if ranking.index(neg["owner"]) > ranking.index(slug):
                misses.append(f"{slug} outranks {neg['owner']} on {neg['prompt']!r}")
    assert not misses, "\n".join(misses)
