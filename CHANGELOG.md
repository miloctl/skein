...
- 1:1 briefs: a teammate's brief opens only under a 1:1 pairing they accepted. On the People page, ask a teammate for a pairing, or let a teammate prepare 1:1s with you. Either side can end it, and the subject sees each lead and when they last opened the brief (`/api/private/pairs`). Your own brief and your private 1:1 notes need no pairing.
- Shared chats: a new room agent reads messages from its join point. The steward can tick "Also share the earlier messages" when adding it, and the room's system message tells every member which. `POST /api/shared-chats/{id}/agents` takes `share_history` as true or false (false by default). Agents added before this release keep the whole history.
