...
export default function Agents() {
  ...
  const load = useCallback(() => {
    ...
    const fail = (key: string, what: string) => (e: Error) =>
      setErrors((cur) => ({
        ...cur,
        [key]: `Cannot load ${what}. ${actionError(e)}`,
      }));
    const ok =
      <T,>(set: (v: T) => void, key: string) =>
      (v: T) => {
        set(v);
    ...
    api<{ id: number; topic: string; content: string; user: string }[]>(
      "/api/memories",
    )
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <Card title="Authority — what each agent can do alone">
          ...
          <p className="mb-2 text-xs text-ink-3">
            In a flock, only <b>not allowed</b> still applies from the levels
            below: every other level becomes a wait in Inbox → Approvals. Skein
            also stops a flock member that tries to work a delegated task or
            make a handoff.
          </p>
          {agents === null ? (
          ...
          {manage && (
            <div className="flex flex-wrap items-center gap-2 border-t border-line pt-2 text-sm">
              ...
              >
                {(entities.length
                  ? entities
                  : [{ entity: "task", label: "add or change a task" }]
                ).map((e) => (
        ...
        >
          {traces === null ? (
            ...
            <ul
              className="max-h-[36rem] space-y-6 overflow-y-auto pr-1 focus-visible:outline focus-visible:outline-2 focus-visible:outline-thread-solid"
              tabIndex={0}
              aria-label="Flock turns, newest first"
            >
              {traces.map((t) => (
                <li key={t.id}>
                  ...
                  <div
                    ...
                    /* not "scrollable": whether it scrolls depends on the
                     viewport, and a label that claims it on a wide screen
                     describes something the reader cannot find */
                    aria-label={`${t.flock} flock diagram`}
                  >
