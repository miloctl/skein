...
resolver fails closed. The rule applies when the application composes an
identity, policy, tool, or workflow action contribution, because each of
them receives the requester. If it composes none of them, no code reads the
groups. Skein then removes the saved groups instead of refusing the
requester.
...
raises when the directory cannot answer. The resolver calls of one refresh
share 2 seconds, because an approval can refresh twice while it holds the
rows it changes. A raise or a timeout answers 503 with `Retry-After`, because the same
...
This revision declares package line `0.6.6`. Registry pull-back and annotated tag `v0.6.6` are the authority for completed publication.
...
```sh
pip install skein-agents==0.6.6 \
...
```json
{
  ...
  "dependencies": {
    ...
    "@miloctl/skein-frontend-host": "0.6.6",
    "next": "16.3.4",
    "react": "19.2.4",
    "react-dom": "19.2.4"
  },
...
It removes both old distributions. Then it installs `skein-agents` 0.6.6 and
