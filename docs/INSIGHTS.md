...
24. **Interrupt load** — at or past `INTERRUPT_SHARE_ALARM` (50%) of a week's finished work was never planned before that week, n≥8 (`portfolio.py::VERDICT_FLOOR_N` returns `None` below the floor, and the rule short-circuits on it). The interrupt ledger shipped with the planning cockpit and nothing read it, so the number reached whoever opened that page on a Monday and nobody else. Team ratio, no per-person split. Severity medium; subject = the ISO week.

25. **Evidence gap** — a DELEGATED task accepted in the last 7 days with zero
