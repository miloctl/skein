...
| Full-text search | FTS5 over every entity; optional embeddings (`SKEIN_EMBEDDINGS=1` **plus `OPENAI_API_KEY`** — the one place still tied to a specific provider) | `GET /api/search?q=` · CLI `skein search` |
