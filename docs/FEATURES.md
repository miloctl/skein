...
| **Tamper-evident provenance** | every `activity` row written since migration 036 carries `seq` + `hash` + `prev_hash`: SHA-256 over its own fields chained to the row before it, so an edited or deleted row breaks verification at a known seq. No backfill — pre-036 rows report as **unchained**, never as verified, because a chain computed today proves nothing about yesterday. `GET /api/activity/verify` walks the whole chain (`?tail=1` resumes from the last verified anchor); `/health` carries `activity_chain`; nightly 03:30 job advances the anchor; the `activity_chain_broken` findings rule pays for a full walk daily. `services/activity.py` + `db.activity_hash` |
