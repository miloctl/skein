...
| Auth model | `SKEIN_AUTH_MODE` picks the door, `routes/deps.py` is the single branch point: `trusted-header` (default — self-asserted `X-User` name picker, LAN/dev; optional `SKEIN_API_TOKEN` shared bearer; boot warns) · `api-key` (every request needs a personal `sk-skein-…` key) · `oidc` (IdP-issued JWTs validated in-process in `app/oidc.py` — JWKS via issuer discovery or `SKEIN_OIDC_JWKS_URL`, asymmetric algorithms only, then iss/aud/exp; keys still work for automation). An unknown mode fails CLOSED (503 on every `/api` request, `/health` says why). Admin surfaces (roster, key visibility/kill switch, authority, team theme, context strategy, backup, export) take `AdminUser`: named in `SKEIN_ADMINS`, or in the `SKEIN_OIDC_ADMIN_GROUP` IdP group; with neither set, trusted-header mode falls back to every-key-holder (the scarcity model), the other modes stay locked. Perimeter middleware in `main.py` enforces the mode even on a route that forgets its user dependency |
