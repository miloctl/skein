...
| Availability ledger | PTO/on-call/focus windows (absences table): capacity marks people away, the weekly draft skips anyone away 3+ weekdays (listed as skipped, never silently), what-if staffing shows upcoming PTO; manage on Work → Browse Time-away card | `GET/POST/DELETE /api/absences` · tools `add_absence`/`list_absences` (create review-gated) |
| Delegation work loop | claim → report_progress (worklog) → submit_for_acceptance: acceptance is ALWAYS a proposal the sponsor approves (verdict marks done + feeds trust); worklog readable pre-verdict | tools `claim_delegated_task`/`report_progress`/`submit_for_acceptance` · `GET /api/tasks/{id}/worklog` |
| Authority review job | weekly scan turns strong-verdict streaks into FILED proposals: 5 straight approvals at review → propose notify; 3 straight rejections at notify/autonomous → propose demotion to review; agents can never approve, so no self-promotion | job `authority-review` · registry entity `authority` |
