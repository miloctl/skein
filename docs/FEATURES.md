...
stay reachable by any LAN caller — only authority editing requires an
...
| Feature | How |
|---|---|
| Per-teammate API keys | first key per person via `python -m app.bootstrap_key <name>` (out-of-band — minting via `POST /api/keys` requires an existing key, or any LAN caller could become anyone); `sk-skein-…` hashed, shown once; authenticates *and* attributes; the STRONG identity for private surfaces; presented-but-revoked keys hard-401. Admin: `GET /api/admin/keys`, `POST /api/admin/keys/revoke-all` (administrators only) |
...
| Auth model | `SKEIN_AUTH_MODE` picks the door, `routes/deps.py` is the single branch point: `trusted-header` (default — self-asserted `X-User` name picker, LAN/dev; optional `SKEIN_API_TOKEN` shared bearer; boot warns) · `api-key` (every request needs a personal `sk-skein-…` key) · `oidc` (IdP-issued JWTs validated in-process in `app/oidc.py` — JWKS via issuer discovery or `SKEIN_OIDC_JWKS_URL`, asymmetric algorithms only, then iss/aud/exp; keys still work for automation). An unknown mode fails CLOSED (503 on every `/api` request, `/health` says why, and the rejected value goes to the log rather than the body). Token validation is hardened against the fact that a `kid` is attacker-chosen and read BEFORE any signature check: fetches carry a 5s timeout, an unknown-kid JWKS refresh is throttled to one per minute, a failed discovery is remembered for 30s, and the middleware runs all of it off the event loop — otherwise a stream of forged kids freezes the server and floods the IdP. In the locked modes `/docs`, `/redoc` and `/openapi.json` are closed too, since they sit outside `/api` where the perimeter cannot see them. Admin surfaces (roster, key visibility/kill switch, authority, team theme, context strategy, backup, export) take `AdminUser`: named in `SKEIN_ADMINS`, or in the `SKEIN_OIDC_ADMIN_GROUP` IdP group; with neither set, trusted-header mode falls back to every-key-holder (the scarcity model), the other modes stay locked. Perimeter middleware in `main.py` enforces the mode even on a route that forgets its user dependency |
...
`_GROUPS_CLAIM`, `_ADMIN_GROUP`), `SKEIN_API_TOKEN`,
