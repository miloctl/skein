...
| **Long-chat memory** | selectable per deployment: `SKEIN_CONTEXT_STRATEGY=sliding` (default — drop the oldest messages, free) or `summarize` (condense them, costs one extra model call when it fires). `SKEIN_CONTEXT_PIN_FIRST=N` pins the opening N messages under BOTH strategies so the framing set at the top of a chat is never dropped or summarized away — this is why Skein does not re-inject context after a compaction, which would add tokens back straight after a trim and can retrigger it. `SKEIN_CONTEXT_PROACTIVE=1` compresses at 70% of the window instead of waiting for an overflow. Real providers only: mock builds no Strands agent, so the strip shows no strategy rather than claiming one. A bad value degrades to sliding and surfaces `context_error` — the same discipline as `MODEL_PROVIDER_ERROR`, never a failed import | `_conversation_manager()` · `GET /api/agents/status` · `/health` |
