...
| Themes | 3 orthogonal axes, all per-browser in Settings: fabric pack (Loom/Ledger/Phosphor/High-contrast — surfaces, texture, type) × colorway (4 presets + Custom two-hue-dial mode, AA at every hue) × appearance (system/light/dark). Pack surfaces respect a brightness invariant so every accent stays AA on every pack. Tokens use CSS `light-dark()` + OKLCH — needs Chrome 123+/Safari 17.5+/Firefox 120+ (older browsers render unstyled) | Settings · `frontend/lib/theme.ts` |
