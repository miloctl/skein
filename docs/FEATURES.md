...
| Chat history + folders | Sidebar on /chat: every conversation is saved (provider-agnostic transcript in `chat_threads`/`chat_messages`, written by the chat route — works keyless), auto-titled from the first message, rename/move-to-folder/delete per thread; switching chats rehydrates the transcript; deleting removes transcript AND model session files; threads are owner-scoped; `fb:` lines never reach the transcript |
