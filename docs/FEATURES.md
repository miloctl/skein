...
| Chat history + folders | Sidebar on /chat: every conversation is saved (provider-agnostic transcript in `chat_threads`/`chat_messages`, written by the chat route — works keyless), auto-titled from the first message, rename/move-to-folder/delete per thread; switching chats rehydrates the transcript; deleting removes transcript AND model session files; threads are owner-scoped by trusted-LAN identity (not private — fb:-grade content belongs in ⌘K/People, and the route refuses it); transcripts live in platform.db, so daily backups retain deleted chats up to 14/30 days; JSON export excludes chat tables |
