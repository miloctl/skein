...
Sign out revokes the server session and expires the cookie first, without contacting the identity provider. Then, in `oidc` mode, if the provider's discovery document names an `end_session_endpoint`, the browser goes there with the client id and `post_logout_redirect_uri` set to the app origin followed by `/`, so the provider's own sign-in session ends too (RP-initiated logout). The session's ID token rides along as `id_token_hint` when the session still holds one and `SKEIN_OIDC_AUDIENCE` differs from the client id: an ID token's audience is the client, so with the two equal it would pass as an API bearer, and the URL shows it in the address bar. Without the hint, the provider can ask the person to confirm. A sign-out after the session expired still reaches the provider. The ID token is kept sealed with the other provider tokens, and a refresh that omits one keeps the token from sign-in. The endpoint comes from discovery only, never from a setting, because the URL carries the ID token. If the provider publishes no endpoint, or discovery fails, sign-out stays local. Browser tabs serialize requests that change cookies. An old callback cannot restore a completed logout.
