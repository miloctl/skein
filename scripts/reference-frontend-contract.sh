...
if [ -n "$release_dist" ]; then
    ...
    cp "$release_dist/miloctl-skein-frontend-host-0.5.0.tgz" "$tmp/tarballs/"
else
...
cp "${host_tar[0]}" "$tmp/consumer/dist/miloctl-skein-frontend-host-0.5.0.tgz"
...
JS
    ...
    cp dist/miloctl-skein-frontend-host-0.5.0.tgz host-tarball.saved
    printf '\nchanged-after-lock\n' >>dist/miloctl-skein-frontend-host-0.5.0.tgz
    expect_build_refusal changed-package-bytes "package bytes do not match the workplace lock."
    mv host-tarball.saved dist/miloctl-skein-frontend-host-0.5.0.tgz
