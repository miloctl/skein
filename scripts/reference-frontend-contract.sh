...
if [ -n "$release_dist" ]; then
    ...
    cp "$release_dist/miloctl-skein-frontend-host-0.6.2.tgz" "$tmp/tarballs/"
else
...
cp "${host_tar[0]}" "$tmp/consumer/dist/miloctl-skein-frontend-host-0.6.2.tgz"
...
JS
    ...
    cp dist/miloctl-skein-frontend-host-0.6.2.tgz host-tarball.saved
    ...
    mv host-tarball.saved dist/miloctl-skein-frontend-host-0.6.2.tgz
