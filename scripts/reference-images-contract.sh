...
if [ -n "${SKEIN_RELEASE_DIST:-}" ]; then
    cp "$SKEIN_RELEASE_DIST/skein_agents-0.6.2-py3-none-any.whl" "$extension/dist/"
    cp "$SKEIN_RELEASE_DIST/miloctl-skein-extension-api-1.0.0.tgz" "$extension/dist/"
    cp "$SKEIN_RELEASE_DIST/miloctl-skein-frontend-host-0.6.2.tgz" "$extension/dist/"
else
...
assert version("skein-agents") == "0.6.2"
