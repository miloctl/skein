...
for overlay in example-prod example-dev; do
  ...
  if [ "$overlay" = example-prod ]; then
    grep -Fq 'image: registry.example.com/skein/skein:0.3.2@sha256:' "$rendered"
    grep -Fq 'image: registry.example.com/skein/skein-frontend:0.3.2-prod@sha256:' "$rendered"
  fi
