...
grep -q "skein_agents-0.6.1-py3-none-any.whl" "$deployment/Dockerfile"
...
for overlay in example-prod example-dev; do
  ...
  if [ "$overlay" = example-prod ]; then
    grep -Fq 'image: registry.example.com/skein/skein:0.6.1@sha256:' "$rendered"
    grep -Fq 'image: registry.example.com/skein/skein-frontend:0.6.1-prod@sha256:' "$rendered"
  fi
