...
- **A new setting lands in one tier, by rule.** Four questions, in order.
  ...
  `agents/mcp_tools.py`). Per-person MCP tokens and browser OIDC credentials
  use the sealed stores in `mcp_servers` and `browser_sessions.sealed_tokens`.
  They are Fernet-sealed under `SKEIN_CREDENTIAL_KEY`, which lives in the
  Secret (`services/credentials.py`). A backup carries MCP ciphertext only;
  browser-session rows are omitted so recovery requires a new sign-in.
  Neither store permits credentials in `app_settings`. There is no combined settings
...
./scripts/lint.sh   # all lint gates CI runs: ruff (check + format) + mypy +
                    ...
                    # + STE self-test and knot how: check + tsc + eslint + knip
```
